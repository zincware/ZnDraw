...
class ZnDraw(MutableSequence[ase.Atoms]):
    ...
    url: str | None = None
    room: str = ""  # composed "<display_name>/<name>" form; empty triggers auto-room
    user: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        # Resolve and validate room in composed form: <display_name>/<room_name>
        raw_room = resolved.room
        if raw_room is None:
            # Auto-room: fetch current user and compose <display_name>/<uuid4()>
            with httpx.Client(base_url=self.url, timeout=30.0) as _client:
                ...
                _display_name = _resp.json()["display_name"]
            raw_room = f"{_display_name}/{uuid.uuid4()}"
        if "/" not in raw_room:
            raise ValueError(
                "Room must be in composed form '<display_name>/<name>'; "
                f"got '{raw_room}'."
            )
