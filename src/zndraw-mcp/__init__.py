...
def upload(path: str) -> tuple[bool, str]:
    ...
    if not is_running:
        return False, f"Cannot upload: ZnDraw server is not running. Status: {status_message}"
...
@mcp.tool()
def run_zndraw_script(python_code: str, url: str|None = None, room: str = "default", user: str|None = None, password: str|None = None) -> tuple[bool, str]:
    """
