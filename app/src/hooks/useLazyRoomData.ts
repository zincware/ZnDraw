...
export const useLazyRoomData = (roomId: string | null, enabled: boolean) => {
	...
	const { data: geometrySchemasData, isLoading: isLoadingSchemas } =
		useGeometrySchemas(enabled && roomId ? roomId : null);
	...
	useEffect(() => {
		...
		if (roomInfoError) {
			console.error(
				"[useLazyRoomData] Failed to load room info:",
				roomInfoError,
			);
		}
		if (selectionsError) {
			console.error(
				"[useLazyRoomData] Failed to load selections:",
				selectionsError,
			);
		}
		if (frameSelectionError) {
			console.error(
				"[useLazyRoomData] Failed to load frame selection:",
				frameSelectionError,
			);
		}
		if (stepError) {
			console.error(
				"[useLazyRoomData] Failed to load current step:",
				stepError,
			);
		}
		if (bookmarksError) {
			console.error(
				"[useLazyRoomData] Failed to load bookmarks:",
				bookmarksError,
			);
		}
		if (geometriesError) {
			console.error(
				"[useLazyRoomData] Failed to load geometries:",
				geometriesError,
			);
		}
	}, [
