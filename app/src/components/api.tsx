...
export function setupBookmarks(token: string, setBookmarks: any, bookmarks: any) {
...
export function setupPoints(token: string, setPoints: any, points: any) {
    ...
    useEffect(() => {
        const updateCon = async () => {
            ...
            conInterface.clear();
            await conInterface.setitem(0, points.map((vec: THREE.Vector3) => [vec.x, vec.y, vec.z]));
            // TODO: set segments
        };
...
export function setupSelection(token: string, setSelection: any, selection: any) {
    let [conInterface, setConInterface]: any = useState(undefined);
    useEffect(() => {
        const con = new znsocket.Dict({
            client: client,
...
export function setupCamera(token: string, cameraPosition: any, orbitControlsTarget: any, setCameraPosition: any, setOrbitControlsTarget: any, controlsRef: any, cameraRef: any) {
    let [conInterface, setConInterface]: any = useState(undefined);
    useEffect(() => {
        const con = new znsocket.Dict({
            client: client,
