...
export const SimulationCell = () => {
    ...
    const [displayedVertices, setDisplayedVertices] = useState<THREE.Vector3[][] | null>(null);
    ...
    useEffect(() => {
        ...
        if (isSuccess && data) {
            ...
            const v = [
                origin, cellVectors[0], cellVectors[1], cellVectors[0].clone().add(cellVectors[1]),
                cellVectors[2], cellVectors[0].clone().add(cellVectors[2]), cellVectors[1].clone().add(cellVectors[2]),
    ...
    const lineColor = mode === 'light' ? theme.palette.primary.dark : theme.palette.primary.light;

    return (
        <group>
            {displayedVertices && displayedVertices.map((points, index) => (
                <Line
                    key={index}
                    points={points}
                    color={lineColor}
