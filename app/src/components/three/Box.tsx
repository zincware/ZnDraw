...
export default function Box({
  ...
  const persistPositions = useCallback(async () => {
    ...
    try {
      await createGeometry(roomId, geometryKey, "Box", currentGeometry.data);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Critical error for Box: position query failed
    const hasCriticalError = typeof positionProp === "string" && isPositionError;

    if (hasCriticalError) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn(
          `Box geometry "${geometryKey}": Critical data fetch failed. Disabling geometry.`,
      ...
      updateGeometryActive(roomId, geometryKey, "Box", false)
        ...
        .catch((error) => {
          console.error(`Failed to disable Box geometry "${geometryKey}" on server:`, error);
