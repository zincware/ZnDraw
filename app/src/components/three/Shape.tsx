...
export default function Shape({
  ...
  const fullData = getGeometryWithDefaults<ShapeData>(data, "Shape", geometryDefaults);
  ...
  const {
    ...
    enabled:
      !!roomId && !!userName && frameCount > 0 && typeof positionProp === "string",
  ...
  const {
    ...
    enabled:
      !!roomId && !!userName && frameCount > 0 && typeof rotationProp === "string",
    placeholderData: keepPreviousData,
    retry: false,
  });
  ...
  const finalPositionData =
    typeof positionProp === "string" ? positionData?.[positionProp] : positionProp;
  useGeometryEditing(geometryKey, finalPositionData, selectedIndices, "Shape", fullData);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchedColor =
        typeof colorProp === "string" ? colorData?.[colorProp as string] : undefined;
  ...
  return (
    <group>
      ...
      <instancedMesh
        ...
        onPointerEnter={
          !pathtracingEnabled && hovering?.enabled ? onPointerEnterHandler : undefined
        }
        onPointerOut={
          !pathtracingEnabled && hovering?.enabled ? onPointerOutHandler : undefined
        }
      >
