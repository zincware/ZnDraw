...
const VectorField: React.FC<VectorFieldProps> = ({ vectors, showArrows = true }) => {
  return (
    <>
      {vectors.map((vector, index) => (
        <React.Fragment key={index}>
          ...
          {showArrows && vector.length > 1 && (
            <ArrowHelper start={vector[vector.length - 2]} end={vector[vector.length - 1]} />
          )}
        </React.Fragment>
      ))}
    </>
...
const ArrowHelper: React.FC<ArrowHelperProps> = ({ start, end }) => {
  ...
  const arrowHelper = new THREE.ArrowHelper(dir, new THREE.Vector3(...start), length, 0x0000ff);
