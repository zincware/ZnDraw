...
const SidebarMenu = ({
    ...
    useEffect(() => {
        ...
        const sharedCon = new znsocket.Dict({ client, key: `schema:default:${name}` });
        ...
        sharedCon.entries().then((items: any) => setSharedSchema(Object.fromEntries(items)));
        queue.length().then((length: any) => setDisabledBtn(length > 0));
        
        const onQueueRefresh = async () => setDisabledBtn(await queue.length() > 0);
        const onSchemaRefresh = async () => setSchema(Object.fromEntries(await con.entries()));
        const onSharedSchemaRefresh = async () => setSharedSchema(Object.fromEntries(await sharedCon.entries()));
    ...
    const fullSchema = useMemo(() => ({ ...sharedSchema, ...schema }), [sharedSchema, schema]);
    const currentSchema = useMemo(() => userInput ? fullSchema[userInput] : null, [fullSchema, userInput]);
    ...
    const debouncedSubmit = useCallback((value: any) => {
    ...
    const cancelTask = useCallback(() => queueRef.current?.clear().then(() => setDisabledBtn(false)), []);
    const capitalizeFirstLetter = useCallback((text: string) => text ? text.charAt(0).toUpperCase() + text.slice(1) : "", []);
