...
const dtypeToConstructor: Record<string, TypedArrayConstructor> = {
	...
	b: Int8Array,
	B: Uint8Array,
	b1: Uint8Array, // Boolean type (1 byte per value, 0=false, nonzero=true)
	i1: Int8Array,
	i2: Int16Array,
	i4: Int32Array,
	i8: BigInt64Array,
...
function reshapeRowMajor(data: any, shape: number[]): any {
	...
	if (shape.length === 2) {
		...
		if (data.length !== totalElements) {
			console.warn(
				`Data length ${data.length} doesn't match shape [${shape.join(", ")}] = ${totalElements}`,
			);
			return data;
		}
	...
	console.warn(
		`Multi-dimensional arrays (${shape.length}D) not fully optimized, returning as-is`,
	);
	...
	if (typeof value === "object") {
		...
		if (isNd !== undefined && dataBytes !== undefined) {
			...
			if (typeof typeStr === "string" && typeStr[1] === "U") {
				...
				const numChars = Number.parseInt(typeStr.slice(2), 10);
				...
				if (endianChar !== "<" && endianChar !== "=" && endianChar !== "|") {
					console.warn(
						`Unsupported endianness for unicode string: ${endianChar}`,
					);
					return undefined;
				}

				// Decode UTF-32LE strings
				const numElements = Array.isArray(shape)
