...
import { useFramePrefetcher } from "../hooks/useFramePrefetcher";
...
function CameraSyncIntegration({
	...
	// Select only the attached camera geometry (not the entire record) to avoid
	// re-running effects when unrelated geometries (other users' cameras) change.
	...
	const { syncToGeometry } = useGeometryCameraSync({
		camera,
		controlsRef,
		controlsState,
	});
	...
	useEffect(() => {
		...
		if (!attachedCameraKey || !attachedCameraGeometry) return;
		if (attachedCameraGeometry.type !== "Camera") return;

		const geomData = attachedCameraGeometry.data;
		if (!geomData) return;
	...
	}, [attachedCameraKey, attachedCameraGeometry, curveRefs, controlsRef]);
