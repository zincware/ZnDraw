...
export default function SiMGenButtons({ onTutorialClick }: SiMGenButtonsProps) {
	...
	// Handler for submitting SiMGen jobs
	const handleJobSubmit = async (jobName: string) => {
		if (!roomId) {
		...
		try {
			// SiMGen jobs are registered as global modifiers
			const fullJobName = `@global:modifiers:${jobName}`;
			await submitTask(roomId, fullJobName, {});
			showSnackbar(`${jobName} submitted successfully`, "success");
		} catch (error) {
			console.error(`Failed to submit ${jobName}:`, error);
			showSnackbar(`Failed to submit ${jobName}`, "error");
		}
	};
		...
		>
			<Tooltip title="Connect selected atoms">
				<Button
					color="inherit"
					startIcon={<LinkIcon />}
					onClick={() => handleJobSubmit("Connect")}
					size="medium"
					sx={{
						textTransform: "none",
						fontWeight: 500,
			...
			<Tooltip title="Run SiMGen Demo">
				<Button
					...
					onClick={() => handleJobSubmit("SiMGenDemo")}
					size="medium"
					sx={{
						textTransform: "none",
						fontWeight: 500,
			...
			<Tooltip title="Replace scene with empty canvas">
				<Button
					...
					onClick={() => handleJobSubmit("NewCanvas")}
					size="medium"
					sx={{
						textTransform: "none",
						fontWeight: 500,
