...
export default function Arrow({
	...
	const { data: colorData, isFetching: isColorFetching } = useQuery({
		...
		queryFn: () =>
			getFrameBatched(roomId!, currentFrame, color as string, queryClient),
		enabled:
			!!roomId &&
			!!userName &&
			frameCount > 0 &&
	...
	const { data: radiusData, isFetching: isRadiusFetching } = useQuery({
		...
		queryFn: () =>
			getFrameBatched(roomId!, currentFrame, radius as string, queryClient),
		enabled:
	...
	const frameKeys = useMemo(() => {
		...
		if (typeof positionProp === "string") keys.push(positionProp);
		if (typeof directionProp === "string") keys.push(directionProp);
		if (typeof color === "string" && shouldFetchAsFrameData(color as string))
			keys.push(color);
		if (typeof radius === "string") keys.push(radius);
		if (typeof scale === "string") keys.push(scale);
		return keys;
	}, [positionProp, directionProp, color, radius, scale]);
