...
export default function Sphere({
	...
	const transformQueries = useQueries({
		queries: allTransformKeys.map((key) => ({
			...
			queryFn: () => getFrameBatched(roomId!, currentFrame, key, queryClient),
	...
	const frameKeys = useMemo(() => {
		const keys: string[] = [];
		// Transform source keys
		for (const k of allTransformKeys) keys.push(k);
		// Direct attribute keys
		if (typeof positionProp === "string" && !positionIsTransform)
			keys.push(positionProp);
		if (
			typeof colorProp === "string" &&
			!colorIsTransform &&
			...
			keys.push(colorProp);
		if (typeof radiusProp === "string" && !radiusIsTransform)
			keys.push(radiusProp);
