 ...
 */
function getCylinderCount(
	bondOrder: number | null,
	mode: "parallel" | "ignore",
): number {
...
function bondStateReducer(
	state: BondState,
	action: BondStateAction,
): BondState {
	switch (action.type) {
		case "UPDATE_ALL":
			return {
...
export default function Bonds({
	...
	const bondInstanceRanges = useMemo(() => {
		...
		for (
			let bondPairIndex = 0;
			bondPairIndex < bondPairs.length;
			bondPairIndex++
		) {
	...
	const {
		...
		queryFn: () =>
			getFrameBatched(roomId!, currentFrame, radiusProp as string, queryClient),
		enabled:
			!!roomId &&
			!!userName &&
			frameCount > 0 &&
	...
	const {
		...
		isFetching: isConnectivityFetching,
		...
		enabled:
			...
			!!userName &&
			frameCount > 0 &&
			typeof connectivityProp === "string",
		placeholderData: keepPreviousData,
		retry: false,
	});
	...
	// Register frame data keys needed by this geometry for prefetching
	...
	useEffect(() => {
		...
		try {
			...
			const fetchedRadius =
				typeof radiusProp === "string"
					? radiusData?.[radiusProp as string]
					: undefined;
			...
			const fetchedConnectivity =
				typeof connectivityProp === "string"
					? connectivityData?.[connectivityProp as string]
					: undefined;
			...
			if (connectivityRaw && connectivityRaw.length > 0) {
				if (Array.isArray(connectivityRaw[0])) {
					...
					(connectivityRaw as Array<[number, number, number | null]>).forEach(
						(conn) => {
			...
			for (
				let bondPairIndex = 0;
				bondPairIndex < newBondPairs.length;
				bondPairIndex++
			) {
			...
			for (const bond of newBondPairs) {
				...
				const radiusScale =
					bond_order_mode === "parallel"
						? (bond_order_radius_scale[bondOrderKey1] ??
							bond_order_radius_scale[bondOrderKey2] ??
							1.0)
						: 1.0;
				...
				for (let cylIdx = 0; cylIdx < cylinderCount; cylIdx++) {
					...
					if (cylinderCount === 2) {
						...
						offsetAmount =
							(cylIdx === 0 ? -0.5 : 0.5) * bond_order_offset * radius;
	...
	useEffect(() => {
		if (!selecting.enabled || !mainMeshRef.current || !selectionMeshRef.current)
			return;
		...
		for (
			let arrayIndex = 0;
			arrayIndex < selectedBondIndices.length;
			arrayIndex++
		) {
	...
	useEffect(() => {
		if (!hovering?.enabled || !mainMeshRef.current || !hoverMeshRef.current)
			return;
		...
		for (
			let arrayIndex = 0;
			arrayIndex < hoveredBondIndices.length;
			arrayIndex++
		) {
	...
	const onClickHandler = useCallback(
		(event: any) => {
			...
			const bondPairIndex =
				instanceToBondMap[event.instanceId] ?? Math.floor(event.instanceId / 2);
	...
	const onPointerEnter = useCallback(
		(e: any) => {
			...
			const bondPairIndex =
				instanceToBondMap[e.instanceId] ?? Math.floor(e.instanceId / 2);
	...
	return (
		<group ref={parentGroupRef}>
			...
			<instancedMesh
				...
				onPointerEnter={
					!pathtracingEnabled && hovering.enabled ? onPointerEnter : undefined
				}
				onPointerMove={
					!pathtracingEnabled && hovering.enabled ? onPointerMove : undefined
				}
				onPointerOut={
					!pathtracingEnabled && hovering.enabled ? onPointerOut : undefined
				}
			>
