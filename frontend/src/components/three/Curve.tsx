...
export default function Curve({
	...
	const [virtualMarkerPositions, setVirtualMarkerPositions] = useState<
		THREE.Vector3[]
	>([]);
	...
	const drawingPointerPosition = useAppStore(
		(state) => state.drawingPointerPosition,
	);
	...
	const handleVirtualMarkerClick = useCallback(
		(event: any) => {
			if (event.instanceId === undefined) return;
			event.stopPropagation();
	...
	const getLineColor = useCallback(
		(colorValue: string) => {
			return colorValue === "default" ? theme.palette.text.primary : colorValue;
		},
		[theme],
	);
	...
	useEffect(() => {
		...
		try {
			...
			const shouldPreserveState =
				...
				finalCount === 0 &&
				markerCount > 0 &&
				markerPositions.length === markerCount;
			...
			if (!shouldPreserveState) {
				...
				const positionsChanged =
					...
					newMarkerPositions.some(
						(pos, i) => !markerPositions[i] || !pos.equals(markerPositions[i]),
					);
	...
	useEffect(() => {
		if (
			!marker.hovering?.enabled ||
			!hoverMeshRef.current ||
			!mainMeshRef.current
		)
			return;
	...
	useEffect(() => {
		...
		isEditing,
		hasStaticPosition,
		virtual_marker.enabled,
		virtualMarkerPositions.length,
	]);
	...
	useEffect(() => {
		...
		const resolvedColor =
			marker.color === "default" ? theme.palette.secondary.main : marker.color;
	...
	useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (e.key === "Backspace" || e.key === "Delete") {
				...
				const newPoints = markerPositions.filter(
		...
		<group>
			...
			{lineSegments.length >= 2 && (
				<Line
					points={lineSegments}
					color={
						isDrawing && isActiveDrawingTarget && !drawingIsValid
							? "#FF0000"
							: getLineColor(color)
					}
					lineWidth={thickness}
			...
			{marker.enabled && (
				<instancedMesh
					...
					onClick={
						marker.selecting?.enabled && (!isEditing || hasStaticPosition)
							? handleMarkerClick
							: undefined
					}
					onPointerEnter={
						marker.hovering?.enabled && (!isEditing || hasStaticPosition)
			...
			{marker.enabled &&
				...
				isActiveDrawingTarget &&
				drawingPointerPosition && (
					<mesh ref={drawingMarkerMeshRef} onClick={handleDrawingMarkerClick}>
						<primitive object={markerGeometry} attach="geometry" />
						<meshBasicMaterial
