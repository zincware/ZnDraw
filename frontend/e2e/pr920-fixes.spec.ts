...
test.describe("PR #920 fixes", () => {
	test("chat unread badge accumulates while hidden and clears on switch-back", async ({ page, browser }) => {
		await page.goto(`/rooms/${ROOM}`);
		await page.getByTestId("activity-icon-chat").click();
		await expect(page.getByTestId("sidebar-zone-right").or(page.getByTestId("sidebar-zone-left"))).toBeVisible();
		...
		const peerInput = peer.getByTestId("chat-input").or(peer.locator('[data-testid="chat-input"] input'));
		...
		const badge = page.getByTestId("activity-icon-chat").locator(".MuiBadge-badge");
