...
def test_cache_hit(server, s22):
    ...
    _ = vis[:]
    assert len(vis.client._cache) == len(s22)
...
def test_cache_invalidation_single_item_replace(server, s22):
    ...
    # 1. Prime the cache.
    _ = vis[:]
    assert 5 in vis.client._cache
    assert 6 in vis.client._cache
    ...
    assert 5 not in vis.client._cache
    assert 6 in vis.client._cache  # Other keys should remain untouched.
...
def test_cache_invalidation_delete(server, s22):
    ...
    assert vis.client._cache.keys() == {0, 1, 2, 3, 4}
...
def test_cache_invalidation_insert(server, s22):
    ...
    # 1. Prime the cache.
    _ = vis[0:10]
    assert vis.client._cache.keys() == set(range(10))
...
def test_cache_invalidation_bulk_slice_assignment(server, s22):
    ...
    vis.extend(s22)
    _ = vis[:]  # Prime cache
    assert vis.client._cache.keys() == set(range(len(s22)))
...
def test_cache_invalidation_clear_all(server, s22):
    ...
    assert len(vis.client._cache) == 0