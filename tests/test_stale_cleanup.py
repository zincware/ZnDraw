...
# =============================================================================
# ClusterHeartbeat tests
# =============================================================================


def test_cluster_heartbeat_start_creates_new_heartbeat(redis_client):
...
def test_cluster_heartbeat_start_reuses_existing_heartbeat(redis_client):
...
def test_cluster_heartbeat_get_heartbeat_timestamp(redis_client):
...
def test_cluster_heartbeat_get_heartbeat_timestamp_returns_none_when_missing(
    redis_client,
):
...
def test_cluster_heartbeat_is_worker_stale_returns_true_for_old_timestamp(redis_client):
...
def test_cluster_heartbeat_is_worker_stale_returns_false_for_new_timestamp(
    redis_client,
):
...
def test_cluster_heartbeat_is_worker_stale_returns_false_when_no_heartbeat(
    redis_client,
):
...
def test_cluster_heartbeat_stop_stops_thread(redis_client):
...
def test_cluster_heartbeat_double_start_is_safe(redis_client):
    """Test that calling start() twice doesn't create duplicate threads."""
    heartbeat = ClusterHeartbeat(redis_client, ttl_seconds=60)

    ts1 = heartbeat.start()
    thread1 = heartbeat._thread

    ts2 = heartbeat.start()
    thread2 = heartbeat._thread

    try:
        # Should return same timestamp and not create new thread
        assert ts1 == ts2
...
def test_cluster_heartbeat_restart_after_stop(redis_client):
    """Test that heartbeat can be restarted after stop()."""
    heartbeat = ClusterHeartbeat(redis_client, ttl_seconds=60)
...
def test_cluster_heartbeat_minimum_refresh_interval(redis_client):
    """Test that refresh interval has a minimum value."""
    # Very short TTL that would create 0 refresh interval without safeguard
    heartbeat = ClusterHeartbeat(redis_client, ttl_seconds=2)

    # refresh_interval should be at least 5 seconds (the minimum)
    assert heartbeat.refresh_interval >= 5


# =============================================================================
# Stale extension cleanup tests
# =============================================================================


def test_stale_extension_cleanup_removes_stale_worker(redis_client):
...
def test_stale_extension_cleanup_keeps_valid_worker(redis_client):
...
def test_stale_extension_cleanup_mixed_stale_and_valid_workers(redis_client):
...
def test_stale_extension_cleanup_global_extension(redis_client):
...
# =============================================================================
# Stale filesystem cleanup tests
# =============================================================================


def test_stale_filesystem_cleanup_removes_stale_filesystem(redis_client):
...
def test_stale_filesystem_cleanup_keeps_valid_filesystem(redis_client):
...
def test_stale_filesystem_cleanup_global_filesystem(redis_client):
...
def test_stale_filesystem_cleanup_skips_filesystem_without_timestamp(redis_client):
...
# =============================================================================
# Cleanup all stale workers for category tests
# =============================================================================


def test_cleanup_all_stale_workers_for_category_multiple_extensions(redis_client):
...
# =============================================================================
# WorkerStats filtering tests
# =============================================================================


def test_worker_stats_excludes_stale_workers(redis_client):
...
def test_worker_stats_counts_only_valid_workers(redis_client):
...
# =============================================================================
# Job dispatcher filtering tests
# =============================================================================


def test_job_dispatcher_get_available_workers_excludes_stale(redis_client):
...
def test_job_dispatcher_get_available_workers_returns_empty_if_all_stale(redis_client):
