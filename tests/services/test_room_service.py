...
# --- Tests for create_room_auto() ---


def test_create_room_auto_returns_early_if_room_exists(redis_client):
    """create_room_auto returns created=False if room already exists."""
    service = RoomService(redis_client)
    # Create room first
    service.create_room("existing-room", "user1")

    result = service.create_room_auto("existing-room", "user2")
...
def test_create_room_auto_rejects_invalid_room_id(redis_client):
    """create_room_auto returns error for invalid room IDs."""
    service = RoomService(redis_client)

    result = service.create_room_auto("room:with:colons", "user1")
...
def test_create_room_auto_rejects_empty_room_id(redis_client):
    """create_room_auto returns error for empty room ID."""
    service = RoomService(redis_client)

    result = service.create_room_auto("", "user1")
...
def test_create_room_auto_copies_from_room(redis_client):
    """create_room_auto copies from existing room when copy_from is room ID."""
    service = RoomService(redis_client)
    # Create source room with frames
    redis_client.zadd("room:source:trajectory:indices", {"frame0": 0})
    redis_client.set("room:source:current_frame", 0)

    result = service.create_room_auto("new-room", "user1", copy_from="source")

    assert result["created"] is True
    assert result["source"] == "room:source"
    assert result["error"] is None
    assert service.room_exists("new-room")


def test_create_room_auto_returns_error_for_nonexistent_source(redis_client):
    """create_room_auto returns error when source room doesn't exist."""
    service = RoomService(redis_client)

    result = service.create_room_auto("new-room", "user1", copy_from="nonexistent")
...
def test_create_room_auto_uses_default_room(redis_client):
    """create_room_auto copies from default_room when no copy_from specified."""
    service = RoomService(redis_client)
    # Create default room
    redis_client.zadd("room:default:trajectory:indices", {"frame0": 0})
    redis_client.set("room:default:current_frame", 0)
    redis_client.set("default_room", "default")

    result = service.create_room_auto("new-room", "user1")

    assert result["created"] is True
    assert result["source"] == "room:default"
    assert result["error"] is None


# Note: Tests for create_room_auto with template application (e.g., copy_from="water"
# or fallback to "empty" template) require Flask application context and are covered
# in integration tests. The _RoomWriter.extend() method uses get_storage() which
# depends on current_app.
