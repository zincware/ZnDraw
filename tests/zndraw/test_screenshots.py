...
async def test_upload_screenshot(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots/upload",
...
async def test_list_screenshots(client: AsyncClient, session: AsyncSession) -> None:
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots?limit=2&offset=0",
        headers=auth_header(token),
    )
    ...
    response2 = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots?limit=2&offset=2",
        headers=auth_header(token),
    )
...
async def test_get_screenshot(client: AsyncClient, session: AsyncSession) -> None:
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots/{screenshot_id}",
        headers=auth_header(token),
    )
    assert response.status_code == 200
    body = response.json()
...
async def test_get_screenshot_not_found(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots/99999",
        headers=auth_header(token),
    )
...
async def test_request_capture(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/screenshots",
...
async def test_request_capture_rejects_other_users_session(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user_a, room)}/screenshots",
        json={"session_id": target_sid},
