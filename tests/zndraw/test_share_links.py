...
async def test_create_and_use_view_link(
    ...
    await client.post(
        "/v1/rooms",
        json={"room_id": "sl-room-1", "visibility": "private"},
        headers={"Authorization": f"Bearer {owner}"},
    )
    # 404 without share token
    r = await client.get(
        "/v1/rooms/sl-room-1", headers={"Authorization": f"Bearer {guest}"}
    )
    assert r.status_code == 404

    # Create view link
    r = await client.post(
        "/v1/rooms/sl-room-1/share-links",
        json={"access": "view"},
        headers={"Authorization": f"Bearer {owner}"},
    )
...
async def test_list_share_links_requires_manage(
    ...
    await client.post(
        "/v1/rooms",
        json={"room_id": "sl-list-1", "visibility": "public"},
        headers={"Authorization": f"Bearer {owner}"},
    )
    # Owner can list (empty so far)
    r = await client.get(
        "/v1/rooms/sl-list-1/share-links",
        headers={"Authorization": f"Bearer {owner}"},
    )
    assert r.status_code == 200
...
async def test_wrong_room_token_rejected(
    ...
    await client.post(
        "/v1/rooms",
        json={"room_id": "sl-room-4a", "visibility": "private"},
        headers={"Authorization": f"Bearer {owner}"},
    )
    await client.post(
        "/v1/rooms",
...
async def test_revoke_nonexistent_returns_404(
    ...
    await client.post(
        "/v1/rooms",
        json={"room_id": "sl-rn-room", "visibility": "public"},
        headers={"Authorization": f"Bearer {owner}"},
    )
    r = await client.delete(
        f"/v1/rooms/sl-rn-room/share-links/{uuid4()}",
        headers={"Authorization": f"Bearer {owner}"},
    )
    assert r.status_code == 404
