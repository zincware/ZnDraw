...
from zndraw.exceptions import FrameNotFound, ProblemDetail, RoomNotFound, UserNotFound
...
async def test_list_frames_empty_room(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames",
...
async def test_list_frames_with_range(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames?start=1&stop=3",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_list_frames_room_not_found(
    ...
    response = await client.get(
        "/v1/rooms/no-such-owner/nonexistent/frames",
        headers=auth_header(token),
    )
    assert response.status_code == 404

    problem = ProblemDetail.model_validate(response.json())
    assert problem.type in (RoomNotFound.type_uri(), UserNotFound.type_uri())
...
async def test_list_frames_with_indices(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames?indices=1,3",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_list_frames_with_keys_filter(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames?keys=x,z",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_list_frames_with_indices_and_keys(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames?indices=2&keys=a",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_get_frame_not_found(client: AsyncClient, session: AsyncSession) -> None:
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames/99",
        headers=auth_header(token),
    )
    assert response.status_code == 404
...
async def test_get_frame_room_not_found(
    ...
    response = await client.get(
        "/v1/rooms/no-such-owner/nonexistent/frames/0",
        headers=auth_header(token),
    )
    assert response.status_code == 404
...
async def test_get_frame_metadata(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames/0/metadata",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_get_frame_metadata_not_found(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/frames/99/metadata",
        headers=auth_header(token),
    )
    assert response.status_code == 404
...
async def test_get_frame_metadata_room_not_found(
    ...
    response = await client.get(
        "/v1/rooms/no-such-owner/nonexistent/frames/0/metadata",
        headers=auth_header(token),
    )
    assert response.status_code == 404
...
async def test_frames_require_authentication(
    ...
    endpoints = [
        ("GET", f"/v1/rooms/{room_display_address(user, room)}/frames"),
        ("GET", f"/v1/rooms/{room_display_address(user, room)}/frames/0"),
        ("POST", f"/v1/rooms/{room_display_address(user, room)}/frames"),
        ("PUT", f"/v1/rooms/{room_display_address(user, room)}/frames/0"),
        ("PATCH", f"/v1/rooms/{room_display_address(user, room)}/frames/0"),
        ("DELETE", f"/v1/rooms/{room_display_address(user, room)}/frames/0"),
    ]
