...
async def test_list_presets_includes_bundled(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/presets",
...
async def test_list_presets_room_not_found(
    ...
    response = await client.get(
        "/v1/rooms/no-such-owner/nonexistent/presets",
        headers=auth_header(token),
    )
...
async def test_get_bundled_preset(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/presets/matt",
        headers=auth_header(token),
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_get_preset_returns_data(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/presets/test-preset",
        headers=auth_header(token),
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_get_preset_returns_404_for_nonexistent(
    ...
    response = await client.get(
        f"/v1/rooms/{room_display_address(user, room)}/presets/nonexistent",
        headers=auth_header(token),
    )
    assert response.status_code == 404
...
async def test_put_preset_creates_new(
    ...
    response = await client.put(
        f"/v1/rooms/{room_display_address(user, room)}/presets/new-preset",
        json=body,
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_put_preset_updates_existing(
    ...
    response = await client.put(
        f"/v1/rooms/{room_display_address(user, room)}/presets/existing",
        json=body,
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_delete_preset(
    ...
    response = await client.delete(
        f"/v1/rooms/{room_display_address(user, room)}/presets/to-delete",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_apply_bundled_preset(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/matt/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_apply_preset_updates_matching_geometries(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/test-apply/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 200
    result = response.json()
...
async def test_apply_preset_deep_merges_config(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/adjust-fog/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 200
...
async def test_apply_preset_filters_by_geometry_type(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/dim-ambient/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 200
    result = response.json()
...
async def test_apply_preset_emits_geometry_invalidate(
    ...
    await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/enable-fog/apply",
        headers=auth_header(token),
    )
...
async def test_apply_preset_returns_404_for_nonexistent(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/nonexistent/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 404
    assert PresetNotFound.type_uri() in response.json()["type"]
...
async def test_apply_preset_skips_non_matching_geometries(
    ...
    response = await client.post(
        f"/v1/rooms/{room_display_address(user, room)}/presets/fog-only/apply",
        headers=auth_header(token),
    )
    assert response.status_code == 200
