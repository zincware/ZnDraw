...
from zndraw.exceptions import TemplateNotFoundError
import requests
import pytest
...
def promoted_template(server, s22):
    ...
    vis1.append(s22[0])
    requests.post(f"{server}/api/rooms/s22-0/promote", json={"name": "My Template", "description": "A custom template"}).raise_for_status()
    return "s22-0"
...
def test_default_template(server, promoted_template, s22):
    # Set the default
    requests.put(f"{server}/api/templates/default", json={"template_id": promoted_template}).raise_for_status()
...
def test_create_from_non_existing_template(server, promoted_template):
    vis2 = ZnDraw(url=server, room="s22-1", user="user1", template="nonexistent-template")
